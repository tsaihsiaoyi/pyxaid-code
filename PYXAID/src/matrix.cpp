...
void qr(double EPS, int n, matrix &eval, vector<double> &Eval)
{
  ...
  do
  {
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      {
        eval.M[i * n + i] = R.M[i * n + i] * Q.M[i * n + i] + R.M[i * n + (i + 1)] * Q.M[(i + 1) * n + i];
    ...
    for (int i = 0; i < (n - 1); i++)
    {
      if ((fabs(eval.M[i * n + (i + 1)].real()) < EPS) && (fabs(eval.M[i * n + (i + 1)].imag()) < EPS))
      {
        ...
        if (sz_up == 1 && sz_dn == 1)
        { // Here we just finished 2x2 matrix - done
...
matrix exp(matrix &m1, complex<double> scl, double eps)
{
  ...
  *****************************************************************************/
  ...
  {
    cout << "Error in exp: Can not exponentiate non-square matrix\n";
    exit(0);
  }
  int n = m1.n_rows;
  ...
  for (int i = 0; i < n; i++)
  {
    eval.M[i * n + i] = exp(eval.M[i * n + i].real() * scl);
  }
...
matrix sin(matrix &m1, complex<double> scl, double eps)
{
  ...
  *****************************************************************************/
  ...
  for (int i = 0; i < n; i++)
  {
    eval.M[i * n + i] = sin(eval.M[i * n + i].real() * scl);
  }
...
matrix cos(matrix &m1, complex<double> scl, double eps)
{
  ...
  *****************************************************************************/
  ...
  for (int i = 0; i < n; i++)
  {
    eval.M[i * n + i] = cos(eval.M[i * n + i].real() * scl);
  }
  //  evec.direct_inverse(eps,inv_evec);
  return (evec * eval * evec.H());
}
...
matrix pow(matrix &m1, double nn, double eps)
{
  ...
  *****************************************************************************/
  ...
  for (int i = 0; i < n; i++)
  {
    eval.M[i * n + i] = pow(eval.M[i * n + i].real(), nn);
  }
  //  evec.direct_inverse(eps,inv_evec);
  return (evec * eval * evec.H());
}
...
void matrix::inverse(double EPS, matrix &INV, int opt)
{
  ...
  { // actually this is slower version - works only up ~n = 100
...
void matrix::direct_inverse(double EPS, matrix &INV)
{
  ...
  double zero = EPS;
  complex<double> *R_time;
  R_time = new complex<double>[num_of_rows * num_of_cols];
  complex<double> *L_time;
  L_time = new complex<double>[num_of_rows * num_of_cols];
