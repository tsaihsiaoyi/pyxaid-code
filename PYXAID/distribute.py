...
def distribute(Nmin, Nmax, max_steps, submit_templ, exp_files, prefixes, do_submit):
    ...
    while j < njobs:
        # add 1 to avoid merging adjacent runs
        job(Nstart, Nend+1, "job%d" % j, prefixes)
    ...
    if do_submit == 1:
        os.system("qsub %s" % submit_templ)
        # we need to wait some time before submitting a new job - to make sure the memory is available
        time.sleep(10)
    elif do_submit == 2:
        os.system("sbatch %s" % submit_templ)
        time.sleep(10)
