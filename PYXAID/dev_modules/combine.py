...
def combine_old_VASP(en_file, real_file, Ham_re, Ham_im):
    ...
    for i in range(0, sz):
        ...
        for j in range(0, sz):
            if i == j:
                # older VASP-based codes report energies in eV
                line = line + "%15.10f " % (E[i] / 13.60569253)
            else:
...
def combine_all_QE(imin, imax, rt):
    ...
    while i < imax:
        print(i)
        combine_old_QE("%senergy%i" % (rt, i), "%snac%i_re" % (rt, i), "%snac%i_im" % (
            rt, i), "%s0_Ham_%i_re" % (rt, i), "%s0_Ham_%i_im" % (rt, i))
        i = i + 1
...
def combine_all_VASP(en_file, res_dir, imin):
    ...
    while i < imin + sz:
        print(i)
        combine_old_VASP("%senergy%i" % (res_dir, i), "%sreal%i" % (
            res_dir, i), "%s0_Ham_%i_re" % (res_dir, i), "%s0_Ham_%i_im" % (res_dir, i))
        i = i + 1
