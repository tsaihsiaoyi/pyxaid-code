...
K_point::K_point(K_point &k1, int min1, int max1, K_point &k2, int min2, int max2)
{
  ...
  if (max1 >= min2)
  {
    cout << "Warning in K_point constructor: There are several identical bands (MO) in given K_point\n";
  }
...
void wfc::set_latt_vectors(boost::python::list lst)
{
  ...
  {
    cout << "Error: in set_latt_vectors. Too few items in the list : " << sz << " Expected at least 9\n";
    exit(0);
  }
}
...
void wfc::compute_Hprime(int minband, int maxband, std::string filename)
{
  ...
  if (maxband >= nbands)
  {
    cout << "Error in compute_Hprime: maxband>=nbands : maxband = " << maxband << " nbands = " << nbands << endl;
    exit(0);
  }
  if (g_sz != npw)
  {
