...
def average(namdtime, num_states, iconds, opt, MS, inp_dir, res_dir):
    ...
    # Distinct excitations (excite to the same state, may be at different times)
    dist_ex = []
        ...
        for in_ex in dist_ex:
            ...
            while i < len(all_ex):
                if all_ex[i] == in_ex:
                    ...
                    # ===================== me_energies file =========================
                    eE = get_file(inp_dir+"/me_energies", i,
                                  namdtime, num_states, 5, 2)

                    # ======================= out file ===============================
                    pP = get_file(inp_dir+"/out", i,
                                  namdtime, num_states, 3, 2)
                    ...
                    # ===================== me_pop file ==============================
                    cC = get_file(inp_dir+"/me_pop", i,
                                  namdtime, num_states, 3, 2)
                    ce = sum_mult_arrays(eE, cC)
                    #C = add_arrays(C,cC)
                    EC = add_arrays(EC, ce)
                    # Dimension: T x num_macro_states
                    ccC = contract_array(cC, ms)
                    C = add_arrays(C, ccC)
...
#            write_array(res_dir+"/se_pop_ex",in_ex,namdtime,num_states,C,num_runs)
            write_array(res_dir+"/sh_pop_ex", in_ex,
                        namdtime, len(ms), P, num_runs)
            write_array(res_dir+"/se_pop_ex", in_ex,
                        namdtime, len(ms), C, num_runs)
...
# >>>>>>>> Second part Macrostates populations and energies <<<<<<
    if opt == 2 or opt == 12:
        ...
        while i < num_macro_states:
            ...
            print("calculation...")
            j = 0
            # Number of microstates corresponding to given macrostate
            num_runs = len(MS[i])
            ...
            while j < num_runs:
                ...
                pEn = get_file(inp_dir+"/me_energies",
                               MS[i][j], namdtime, num_states, 5, 2)
                # Dimension:  T x num_macro_states
                cpEn = contract_array(pEn, MS)
            ...
            write_array(res_dir+"/energy_macro_ex", i, namdtime,
                        num_macro_states, En, num_runs)
            write_array(res_dir+"/sh_pop_macro_ex", i,
                        namdtime, num_macro_states, P, num_runs)
            write_array(res_dir+"/se_pop_macro_ex", i,
                        namdtime, num_macro_states, C, num_runs)
