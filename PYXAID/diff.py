...
def diff(out_filename, t0, T, at, res_filename, opt):
    ...
    # index of the currently processing position in the internal indexing of small arrays R0,Rt,D
    indx = range(0, sz)
                    ...
                    while i < sz:  # run through all groups of atoms
                        if (at_line+1) in at[i]:  # Found specific atoms
                            ...
                            for j in [0, 1, 2]:  # all components x, y, z
                                if opt == 0:
                                    D[i][indx[i]][j] = 0.5*convert * \
                                        ((Rt[i][indx[i]][j] - R0[i]
                                         [indx[i]][j])**2 / float(t-t0))
                                elif opt == 1:
                                    D[i][indx[i]][j] = 0.5*convert * \
                                        ((Rt[i][indx[i]][j] - Rprev[i]
                                         [indx[i]][j])**2 / float(1.0))
